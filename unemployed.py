...
def findCity (person_location, *args):
    for capital in Coordinates:
        ...
        distance_between_points = geodesic(person_location,capital_city).km
...
for row in postcodes[1:]:
    now= row[0].split()
#searching for LGA codes in the post codes file
for row in unemployment[1:]:
    for area in postcodes[1:]:
        ...
        if row[3] in area_new[1]:
            ...
            #adding the no of unemployed persons for that location to the dictionary
